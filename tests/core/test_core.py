...
class TestFiles:
    ...
    def test_ljh_copy_and_append_traces():
        ...
        global d
        src_name = os.path.join(d, "..", 'regression_test', 'regress_chan1.ljh')
    ...
    def test_ljh_truncate_n_pulses(self):
        ...
        # Tests with a file with 1230 pulses, each 1016 bytes long
        src_name = os.path.join(d,'..', 'regression_test', 'regress_chan3.ljh')
    ...
    def test_ljh_dastard_other_reading():
        ...
        src_name1 = os.path.join(d,'..', 'regression_test', 'regress_chan1.ljh')
        src_name2 = os.path.join(d,'..', 'regression_test', 'regress_dastard_chan1.ljh')
        data1 = mass.TESGroup(src_name1)
        data2 = mass.TESGroup(src_name2)
        for dd in (data1, data2):
            dd.summarize_data()
        ds1 = data1.channel[1]
        ds2 = data2.channel[1]
    ...
    def test_ragged_size_file():
        ...
        mass.LJHFile.open(os.path.join(d,'..',"regression_test","phase_correct_test_data_4k_pulses_chan1.ljh"))
    ...
    def test_peak_time_property():
        ...
        src_name1 = os.path.join(d,'..', 'regression_test', 'regress_noise_chan1.ljh')
...
def test_ljh_file_rownum():
    ...
    src_name = os.path.join(d,'..', 'ljh_files', '20230626', '0001', '20230626_run0001_chan4109.ljh')
...
class TestTESGroup:
    ...
    def load_data(hdf5_filename=None, hdf5_noisefilename=None, skip_noise=False,
        ...
        src_name = [os.path.join(d,'..','regression_test','regress_chan1.ljh')]
        noi_name = [os.path.join(d,'..','regression_test','regress_noise_chan1.ljh')]
        if skip_noise:
            noi_name = None
    ...
    def test_experiment_state(self, tmp_path_factory):
        ...
        esf = os.path.join(d,'..','regression_test','regress_experiment_state_v2.txt')
    ...
    def test_noise_only():
        ...
        pattern = os.path.join(d,'..','regression_test','regress_noise_*.ljh')
    ...
    def test_invert_data(self, tmp_path_factory):
        ...
        for ds2 in data2:
            ds2.invert_data=True
        ds2 = data2.channel[1]
        ds2.summarize_data()
        assert np.allclose(ds.p_pulse_average[:], -ds2.p_pulse_average[:])
        assert ds.peak_samplenumber != ds2.peak_samplenumber, "peak sample number also inverts the data, but since the pulses aren't really inverted, ds2 should get the wrong answer"
    ...
    def test_pulse_model_and_ljh2off(self, tmp_path_factory):
        ...
        _ljh_filenames, off_filenames = mass.ljh2off.ljh2off_loop(
            ds.filename, hdf5_filename, tmp_path_factory.mktemp("2"), max_channels,
        ...
        output_dir = tmp_path_factory.mktemp("3")
    ...
    def test_projectors_script(tmp_path):
        ...
        class Args:
            def __init__(self):
                self.pulse_path = os.path.join(d,'..', 'regression_test', 'regress_chan1.ljh')
                self.noise_path = os.path.join(d,'..', 'regression_test', 'regress_noise_chan1.ljh')
...
"""
        ...
        for have_esf in (False, True):
            data = make_data(have_esf, tmp_path_factory.mktemp(str(have_esf)))
            # data.summarize_data()
            ds = data.channel[1]
            ds.good()
            if have_esf:
