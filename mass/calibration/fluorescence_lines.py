...
class TwoVoigtFitter(object):
    """Fit a pair of Lorentzian lines, with Gaussian smearing.
    ...
    def fit(self, data, pulseheights=None, params=None, plot=True, axis=None, color=None, label="",
        ...
        fitter.setbounds(2, 0, 10*DE) # 200 > Lorentz FWHM 1 > 0
        fitter.setbounds(3, 0, None)  # amplitude 1 > 0
        fitter.setbounds(4, 0, None)  # PH at peak 2 > 0
        fitter.setbounds(5, 0, 10*DE) # 200 > Lorentz FWHM 2 > 0
...
class SimultaneousMultiLorentzianComplexFitter(object):
    ...
    def fit(self, data, pulseheights=None, params=None, plot=True, axis=None, color=None, label="",
        ...
        if params == None:
            params = self.guess_starting_params(data, pulseheights)
        if params[4] == 0:
            params[4] = 1e-7  # the fitter crashes if params[4] bg_level is zero
        print('start params')
        print(params)
...
class MultiLorentzianComplexFitter(object):
    ...
    * a self.guess_starting_params method to return fit parameter guesses given a histogram.
    ...
    def fit(self, data, pulseheights=None, params=None, plot=True, axis=None, color=None,
        ...
        if not vary_tail:
            hold.add(6)
        if 6 in hold and params[6] <= 0.0:
            hold.add(7)
        if 6 not in hold and params[6] <= 0.0:
            params[6] = .01
        if params[4] <= 0:
            params[4] = 1e-6
