...
class MultiExponentialCovarianceSolver(object):
    ...
    def solve(self, b):
        ...
        Return: <x>"""
        if len(b.shape) > 1:
            raise ValueError("solve(b) only works on vectors, not matrices")
        n = len(b)
        if n > self.nsamp:
    ...
    def cholesky_product(self, x):
        ...
        variance, then Lx has expected covariance matrix equal to R"""
        if len(x.shape) > 1:
            raise ValueError("cholesky_product(x) only works on vectors, not matrices")
        n = len(x)
        if n > self.nsamp:
            raise ValueError("The covariance matrix was factored for only "+
    ...
    def unwhiten(self, x):
        """Another name for cholesky_product"""
        return self.cholesky_product(x)
    ...
    def cholesky_solve(self, x):
        ...
        and the product becomes a simple dot product (a' R^-1 b) = A'B."""
        if len(x.shape) > 1:
            raise ValueError("cholesky_solve(x) only works on vectors, not matrices")
        n = len(x)
        if n > self.nsamp:
            raise ValueError("The covariance matrix was factored for only "+
                             "%d samples.  Its Cholesky factor cannot multiply size %d>%d"%(
    ...
    def whiten(self, x):
        """Another name for cholesky_solve"""
        return self.cholesky_solve(x)
...
#        print 'Complex bases: ',complex_bases
    ...
    def results(self):
        ...
        for b in self.complex_bases:
            bases.append(b)
            bases.append(b.conjugate())
            r = stored_amp.pop(0)*0.5
            c = stored_amp.pop(0)*0.5
    ...
    def summary(self, dt=1.0):
        s=['Summary of an exponential sum fit:']
        posneg_bases = np.hstack((self.real_bases, self.negative_bases))
        for i,b in enumerate(posneg_bases):
            rate = np.log(abs(b))/dt
            if b>0:
                s.append("%11.4f exp(%10.5f t)  [x = %10.6f  tau  = %10.4f]" % (self.amplitudes[i], rate, b, -1./rate))
            else:
                s.append("%11.4f exp(%10.5f t)  [x = %10.6f  tau  = %10.4f] (-1)^k" % (self.amplitudes[i], rate, b, -1./rate))
        for i,c in enumerate(self.complex_bases):
            j = i*2 + len(posneg_bases) # includes the negative ones
            rate = np.log(c).real/dt
            freq = np.log(c).imag/dt
            s.append("%11.4f exp(%10.5f t) cos(%10.5f t) tau  = %10.4f samp" % (self.amplitudes[j], rate, freq, -1/rate))
            s.append("%11.4f exp(%10.5f t) sin(%10.5f t) period %10.4f samp" % (-self.amplitudes[j+1], rate, freq, 2*np.pi/freq))
        return "\n".join(s)
