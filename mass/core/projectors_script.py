...
n_basis, maximum_n_pulses, mass_hdf5_path):
    data = mass.TESGroup(pulse_files,noise_files,overwrite_hdf5_file=True,hdf5_filename=mass_hdf5_path)
    data.summarize_data()
    data.auto_cuts(nsigma_pt_rms=n_sigma_pt_rms, nsigma_max_deriv=n_sigma_max_deriv)
    data.compute_noise_spectra()
    data.compute_ats_filter(shift1=False)
    pulse_model_filename=data.pulse_model_to_hdf5(h5, n_basis=n_basis, maximum_n_pulses=maximum_n_pulses)
...
def parse_args(fake):
    ...
    parser.add_argument("--n_sigma_pt_rms", type=float, default=8, help="passed to autocuts to determine pulses used to generate pulse model")
    parser.add_argument( "--n_sigma_max_deriv", type=float, default=8, help="passed to autocuts to determine pulses used to generate pulse model")
    parser.add_argument("-n","--n_basis", type=int, default=5, help="how many projectors to generate, must be >=3")
    parser.add_argument("--maximum_n_pulses", type=int, default=4000, help="maximum number of pulses to take a truncated SVD of, will affect speed")
    parser.add_argument("--silent", action="store_true", help="supress text output, mostly for testing")
    parser.add_argument("--mass_hdf5_path", default=None, help="specify the path for the mass hdf5 file that is generated as a byproduct of this script")
...
def main(args=None): 
    ...
    if not args.silent: print("starting make_projectors")
    for k in sorted(vars(args).keys()):
        print("{}: {}".format(k, vars(args)[k]))
    # find files
    channums = mass.ljh_util.ljh_get_channels_both(args.pulse_path, args.noise_path)
    if not args.silent: print("found these {} channels with both pulse and noise files: {}".format(len(channums),channums))
    if args.max_channels < len(channums):
        channums = channums[:nchan]
        if not args.silent: print("chose first max_channels={} channels".format(args.max_channels))
    if len(channums) == 0:
        raise Exception("no channels found for files matching {} and {}".format(args.pulse_path, args.noise_path))
