...
def process_file(prefix, cuts):
    ...
    channels=(1,)
    pulse_files=["%s_chan%d.ljh"%(prefix, c) for c in channels]
    noise_files=["%s_chan%d.noi"%(prefix, c) for c in channels]
    
    # Start from clean slate by removing ay hdf5 files
    for fl in glob.glob("%s_mass.hdf5" % prefix):
        os.remove(fl)
    for fl in glob.glob("%s_noise_mass.hdf5" % prefix):
        os.remove(fl)
