...
def generate_mnkalphabeta_data(
    ...
    manganese = mass.calibration.fluorescence_lines.MnKAlphaDistribution()
    manganese2 = mass.calibration.fluorescence_lines.MnKBetaDistribution()
...
data.plot_traces(np.arange(15), dataset_num=0)
...
# quantities.  plt lets you zoom and pan with the mouse.  Check them out.
# <demo> --- stop ---
...
medians = np.array([np.median(ds.p_filt_value) for ds in data.datasets])
hist_limits = np.array([.99, 1.01])*medians.mean()
plt.clf()
colors = 'purple', 'blue', 'green', 'gold', 'orange', 'red'
for i, (color, median, ds) in enumerate(zip(colors, medians, data.datasets)):
    good = np.abs(ds.p_filt_value/median-1.0) < .02
    plt.subplot(2, 1, 1+i)
    plt.hist(ds.p_filt_value[good], 200, hist_limits,
...
#    guess Mn KBeta location
    ...
    use = np.logical_and(ds.cuts.good(), use)
