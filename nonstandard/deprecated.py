...
#            print valid_ph.sum(),"   ",
    ...
    def errfunc(p, x, y): return y-model(p, x)

    params = (0., 4, corrections.mean())
    fitparams, _iflag = sp.optimize.leastsq(
        errfunc, params, args=(self.p_filt_phase[valid], ph_vector[valid]))
    phases = np.arange(-0.6, 0.5001, .01)
    if plot:
        plt.plot(phases, model(fitparams, phases), color='blue')
...
#            print "%6.3f %7.2f"%(sl,rms)
    ...
    self.p_filt_value_dc = self.p_filt_value_phc + \
        (self.p_pretrig_mean-mean_pretrig_mean)*best_slope
...
def fitExponentialRiseTime(ts, dt=1.0, nPretrig=0):
    ...
    def fitfunc(p, x): return p[0]*np.exp(-x/p[1])+p[2]
...
#        params[4] = 0.0
    ...
    if update_energy:
        self.p_energy = calib(self.p_filt_value_dc)
